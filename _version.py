...
Generated by talon-pack v3.2.5

This file provides:
- Package version checking via noise_map_version() action
...
def _get_version() -> tuple[int, int, int]:
    ...
    try:
        ...
        print(f"ERROR: talon-noise-map failed to load version from manifest.json: {e}")
        return (0, 0, 0)

@mod.action_class
class Actions:
    def noise_map_version() -> tuple[int, int, int]:
        """
        Returns the package version as (major, minor, patch).

        Usage: actions.user.noise_map_version() >= (1, 2, 0)
        """
        return _get_version()
